...
void HidnSeek::sendMessage(const char* input){
	_serial.write(input);
...
bool HidnSeek::isReady() {
    ...
    if (currentTime >= _lastSend &&
        ((unsigned long)(currentTime - _lastSend) <= 600000UL)) {
        return false;
    }
...
void HidnSeek::flashBlu(int num) {
    while (num > 0) {
        PORTD |= (1 << bluLEDpin);
        delay(50);
        PORTD &= ~(1 << bluLEDpin);
...
void HidnSeek::checkVoltage() {
    if (millis() - _lastCheck > 2000UL) {
        ...
        if (value < 900)
            setSupply(false); // Below 3,7V shutdown the supply
        if (!_grst)
            PORTB &= ~(1 << 2);
    }
}
...
bool GPS::init() {
    ...
    if (GPSready) {
        cmd(PSTR(PMTK_SET_NMEA_OUTPUT));
        cmd(PSTR(PMTK_SET_NMEA_UPDATE_1HZ)); // 1 Hz update rate
    } else
...
bool GPS::process() {
    ...
    // lat: 32, lon: 32, alt: 13 , spd: 7, cap: 2, bat: 7, mode: 3 (0-3 sat view,
    // more is MSG)
    // lat: 32, lon: 32, alt:0-8191m, spd:0-127Km/h, bat:0-100%, mode:0-7, cap:
    // N/E/S/W
    ...
    if (newGpsData) { // computeData
        ...
        sat = gps.satellites() == TinyGPS::GPS_INVALID_SATELLITES
                  ? 0
                  : gps.satellites();
...
void GPS::makePayload() {
    ...
    if (sat > 3) {
        ...
        if (alt > 4096)
            alt = (uint16_t)(alt / 16) + 3840; // 16m step after 4096m
        if (alt > 8191)
            alt = 8191; // 69632m is the new limit ;)

        if (spd > 127)
            spd = (uint16_t)(spd / 16) + 94; // 16Km/h step after 127Km/h
        else if (spd > 90)
            spd = (uint16_t)(spd / 3) + 60; // 3Km/h step after 90Km/h
        if (spd > 126)
            spd = 127; // limit is 528Km/h
        cap = (gps.course() / 90) % 4;
    } else
        cap = (accelPosition < 3) ? accelPosition : 3;
...
void HidnSeek::sendSigFox(byte msgType) {
    GPS gps;
    PORTD |= (1 << redLEDpin);
    // isReady check removed in the library due to reset of millis during sleep
    // time
    gps.makePayload();
    if (msgType > 0) {
        if (baromPresent) {
...
bool HidnSeek::bmp180Measure(float *Temp, unsigned int *Press) {
    if (Temp)
        *Temp = bmp180.bmp085GetTemperature(bmp180.bmp085ReadUT()) - 3.6;
    if (Press) {
        *Press =
            (unsigned int)(bmp180.bmp085GetPressure(bmp180.bmp085ReadUP()) / 100);
...
void HidnSeek::dumpEEprom() {
    ...
    // Display numbers of messages sent to sigfox network on day 1, 2, 3, ... ,
    // 30, 31
...
bool HidnSeek::accelStatus() {
    ...
    if (error != 0)
        ...
        accelMove = ((uint8_t)(abs((int8_t)(accel.getX() - x))) > 2)
                        ? true
                        : ((uint8_t)(abs((int8_t)(accel.getY() - y))) > 2)
                              ? true
                              : ((uint8_t)(abs((int8_t)(accel.getZ() - z))) > 2)
                                    ? true
                                    : false;
        x = accel.getX();
        y = accel.getY();
        ...
        if (accelMove) { // Compute accelDance on move
            byte newPosition;
            if (abs(x) < ACCEL_FLAT && abs(y) < ACCEL_FLAT && z > ACCEL_TRIG)
                newPosition = POS_FACE_UP;
            else if (abs(x) < ACCEL_FLAT && abs(y) < ACCEL_FLAT && z < -ACCEL_TRIG)
                newPosition = POS_FACE_DN;
            else if (abs(x) < ACCEL_FLAT && y < -ACCEL_TRIG && abs(z) < ACCEL_FLAT)
                newPosition = POS_SIDE_UP;
            else if (abs(x) < ACCEL_FLAT && y > ACCEL_TRIG && abs(z) < ACCEL_FLAT)
                newPosition = POS_SIDE_DN;
            else if (x > ACCEL_TRIG && abs(y) < ACCEL_FLAT && abs(z) < ACCEL_FLAT)
                newPosition = POS_SIDE_RT;
            else if (x < -ACCEL_TRIG && abs(y) < ACCEL_FLAT && abs(z) < ACCEL_FLAT)
                newPosition = POS_SIDE_LT;
            else {
                seq = 0;
                newPosition = POS_NULL;
            }
            if (seq == 0 && accelPosition == POS_FACE_UP &&
                newPosition == POS_SIDE_UP) {
                seq++;
                NoflashRed();
            } else if (seq == 1 && accelPosition == POS_SIDE_UP &&
                       newPosition == POS_SIDE_DN) {
                seq++;
                NoflashRed();
            } else if (seq == 2 && accelPosition == POS_SIDE_DN &&
                       newPosition == POS_SIDE_UP) {
                seq++;
                NoflashRed();
            } else if (seq == 2 && accelPosition == POS_SIDE_DN &&
...
bool HidnSeek::batterySense() {
    ...
    if (batteryComp > sensorMax)
        sensorMax = calibrate(batteryComp);
    unsigned int bat = map(batteryValue, 0, sensorMax, 0,
                           BATT_MAX); // represent the battery voltage
